...
// Inline detail panel for a recently-finished design build.
function ExpandedDesign({
  designId, finishedAt, onOpen
}: {
  designId: string
  finishedAt: number
  onOpen?: () => void
  ...
  return (
    <div className="flex flex-col gap-2 px-2 pb-2">
      <div className="rounded-md bg-bg/60 px-3 py-2 text-[12px] text-text-secondary">
        {design ? (
          <div className="flex flex-col gap-1">
            <div><span className="text-text-muted">Title: </span>{design.title}</div>
            {design.currentVersion && (
              <div><span className="text-text-muted">Version: </span>{design.currentVersion}</div>
            )}
            <div><span className="text-text-muted">Finished: </span>{relativeTime(finishedAt)}</div>
          </div>
        ) : (
          <span className="text-text-muted">Loading details…</span>
        )}
      </div>
      {onOpen && (
        <div className="flex justify-end">
          <button
            type="button"
            onClick={onOpen}
            className="rounded-md bg-elevated px-2.5 py-1 text-[11.5px] text-text-primary hover:bg-elevated/70"
          >
            Open design
          </button>
        </div>
